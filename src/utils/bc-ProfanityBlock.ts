/**
 * Originally from https://github.com/The-Best-Codes/bc-ProfanityBlock
 * I converted it to TypeScript using ChatGPT, and adding "export default".
 */
export default class ContentFilterBadWord {
  private encodedCussWords: string[];
  private evasionPatterns: { pattern: RegExp; replacement: string }[];
  ...
  containsBadWords(
    ...
    const cussWords: string[] = this.encodedCussWords.map((encodedWord) =>
    ...
    if (detectEvasionSeparators) {
      ...
      cussWords.forEach((cussWord) => {
        const wordRegex = new RegExp(cussWord.split("").join("\\s*"), "gi");
  ...
  cleanText(
    ...
    if (detectEvasionSeparators) {
      ...
      this.encodedCussWords.forEach((encodedWord) => {
        ...
        const wordRegex = new RegExp(cussWord, "gi");
